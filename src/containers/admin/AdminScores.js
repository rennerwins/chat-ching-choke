...
class AdminScores extends Component {
  ...
  showRandomCorrectUsers = currentQuiz => {
    api.showRandomCorrectUsers(currentQuiz).then(({ answerRate, correctUsers }) => {
      const answers = _.values(answerRate);
      this.setState({
        answerRate: answers,
        correctUsers,
      });
  ...
  showResults = () => {
    ...
    db.ref('currentQuiz').once('value', snapshot => {
          ...
          {this.state.answerRate.map((ans, index) => (
            <div className="row my-4 align-items-center" key={index}>
              ...
              <div className="col-10">
                <BarChart point={ans} /> <h3 className="mb-0 answer-rate">{ans}%</h3>
              </div>
            </div>
          ))}
        </div>
