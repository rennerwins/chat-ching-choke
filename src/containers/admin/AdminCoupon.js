...
class AdminCoupon extends Component {
  ...
  checkCoupon = () => {
    const { couponPair, amount } = this.state;

    if (amount > this.state.matchedUser.length) {
      ...
      <div>
        ...
        <div className="row align-items-center fixed-bottom">
          <div className="col-2 text-center">
            <FormControlLabel
              control={<Checkbox checked={this.state.firstPrize} onChange={this.handleFirstPrize} />}
              label="1st Prize"
            />
          </div>
          ...
          <div className="col-2 text-center">
            <Button raised color="accent" disabled={!this.state.firstPrize} onClick={this.startCountDown}>
              เริ่มจับเวลา
            </Button>
          </div>
